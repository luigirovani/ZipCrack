...
func getBuffer() []byte {
	...
	bufferSize := int64(totalMemory.Available) / 12 / 4
...
func crack(zipFile string, dictFile string) {
	...
	for {
		...
		passwordFound = processData(buffer[:bytesRead], zipFile)
		if passwordFound {
			return
		}
	}
	...
	if !passwordFound {
		fmt.Println("No password found in the dictionary.")
	}
}
...
func main() {
	if len(os.Args) < 4 {
		fmt.Printf("\nUsage: %s [zip file] [dictionary file/letters] [type of attack]\n"+
			...
			"\t- Brute force: %s ExampleFile.zip abcdefghijklmnopqrstuvwxyz1234567890 bruteforce\n\n",
